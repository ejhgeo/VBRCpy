#!/usr/bin/env python3
"""
PREM validation case.

Inverts PREM Vs and Q profiles through a pre-computed VBR sweep to recover
...
--------
  ...
  2. Load PREM Vs and Q, write an observation CSV in model-mode format.
  3. Run the Bayesian inversion on the PREM observations.
...
_output_dir_rel = _cfg.get('output_dir', 'validation_tests/validationCase_PREM')
...
def _make_profile_plots(depth_km, Vs_obs, Q_obs, ml_csv, method='xfit_premelt'):
    ...
    fig_path = os.path.join(OUTPUT_DIR,
                            f'prem_profiles_{method}_{prior_label}_{obs}.png')
...
def main():
    parser = argparse.ArgumentParser(
        description='PREM validation case: invert PREM Vs/Q profiles',
        add_help=True,
    )
    ...
    # ------------------------------------------------------------------
    ref_model = _cfg.get('reference_model', 'prem')
    print("\n" + "=" * 70)
